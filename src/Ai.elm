...
bestMove board player =
    let
        ...
        updatedBoard =
            (\id -> updateBoard id player board)

        rankedBoard =
            (\id -> rankBoard (updatedBoard id) player defaultWinScore)

        mappedMoves =
            List.map (\id -> rankedBoard id) moves

        scores =
            Utils.zipmap moves mappedMoves
        ...
        bestMove =
            List.filter (\x -> (Utils.defaultGet default x scores) == bestScore) moves
    in
        List.head (bestMove)
            |> Maybe.withDefault default
...
rankBoard board player winScore =
    if isWinner board player then
        ...
        let
            ...
            nextBoard =
                (updateBoard (bestMove board opponent) opponent board)
        in
